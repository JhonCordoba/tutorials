...
(define grammar-simple-interpreter
  '((program (expression) a-program)
    ...
    (type-exp ("(" (separated-list type-exp "*") "->" type-exp ")")
              proc-type-exp)
    ;(proc-type-exp (list (int-type-exp) (int-type-exp)) (int-type-exp))
    ;representacion de los tipos de datos de even? (proc-type-exp (list (int-type-exp)) (bool-type-exp))
    ;;;;;;;;
    ))
...
;Representacion del tipo de dato de la funcion suma (int*int->int)
(proc-type-exp (list (int-type-exp) (int-type-exp)) (int-type-exp))
;Tipo de dato para even?
(proc-type-exp (list (int-type-exp)) (bool-type-exp))

(scan&parse "proc(int x, bool y, (int->bool) even) x")
(scan&parse "proc(int x, bool y, (int * int -> int) suma) (suma x y)")
;(scan&parse "proc(x, y, f) (f +(x, y))") tf = (int->?????) Muestra error de tipos se debe trabajar con inferencia de tipo no siempre se conoce todos los tipos
(scan&parse "proc(int x, int y, (int->int)f) (f +(x, y))") ;Chequeo de tipo

(scan&parse "letrec int fact(int x) = if zero?(x) then 1 else *(x, (fact sub1(x))) in (fact 6)")
( type-of-program (scan&parse "letrec int fact(int x) = if zero?(x) then 1 else *(x, (fact sub1(x))) in (fact 6)"))
(type-to-external-form (type-of-program (scan&parse "letrec int fact(int x) = if zero?(x) then 1 else *(x, (fact sub1(x))) in (fact 6)")))

;(type-of-program (scan&parse "if true then 1 else false"))
(type-of-program (scan&parse "proc(int x, bool y) if true then 1 else 2"))
